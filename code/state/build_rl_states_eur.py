# build_rl_states_eur.py
# Converts frozen EURUSD Parquet state → RL-ready NumPy states with ablations
...
STATE_IN = Path(
    "data/processed/state/eur/eurusd_state_with_macro_cnn_30m_2019_2023.parquet"
)
...
OUT_BASE = Path("data/rl_states/eur")
...
def main():
    print("Loading frozen EURUSD state...")
    df = pd.read_parquet(STATE_IN).sort_values("timestamp")
    ...
    candidate_flags = ["burst_flag_24h"] + macro_flag_cols
    ...
    print("\nEUR RL states successfully built:")
    print(" →", OUT_BASE)
