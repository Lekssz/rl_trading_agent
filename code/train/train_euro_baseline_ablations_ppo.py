# PPO EURUSD Baseline Ablations Training Script
import sys
import numpy as np
import pandas as pd
...
EXPERIMENT_NAME = "ppo_eur_baseline_ablations"
...
def evaluate_strategy(model, env):
    ...
    return df, final_return, sharpe, max_drawdown, total_trades
...
def main():
    ...
    for name, state_file in ABLATIONS.items():
        print(f"\n=== Running EURUSD Ablation: {name} ===")

        X = np.load(PROJECT_ROOT / "data/rl_states/eur/npy" / state_file)
